...
import { Button, Modal, Form, Input,Radio ,Divider,InputNumber,DatePicker,Row,Col,Space } from 'antd';
...
const AdvancedForm = memo(function AdvancedForm(props) {
  ...
  const initValue = [{names:'',sex:'',address:'',school:'',age:'',birth:''}];
        ...
        data.map((item,index)=>(
          <div key={index}>
               <Divider>信息{index+1}</Divider>
                <div>
                    {
                      index>0 && <Button type="danger" onClick={()=>removeStemp(index)} shape="circle" icon={<DeleteOutlined style={{color:'#fff'}}/>} className='fr' style={{marginLeft:'20px'}}/>
                    }
                </div>
  ...
  const removeStemp = (k) => { // 删除
    setStepList(stepList.filter((item,index) =>index !== k));
    const userInfoList =form.getFieldValue('userInfoList');
  ...
  const handleSubmit = e => {
    form.validateFields().then(values => {
      ...
      data.userInfoList.forEach(item=>{
        item.birth = item.birth.format('YYYY-MM-DD');
      })
