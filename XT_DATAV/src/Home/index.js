...
import { Pagination, Col, Row, Card, Avatar, Spin, message } from 'antd';
  ...
  const renderCardList = useMemo(() => {
    ...
    return list.map(item => <Col style={{ margin: '10px 0px' }} key={item.id} span={6}>
      <Card
        ...
        extra={[
          <div key="like" style={{ display: 'inline', }}><CopyFilled style={{ fontSize: '16px', color: '#08c' }}/><b style={{ fontSize: 15, marginLeft: 5 }}>{item.star}</b></div>,
          <div key="heart" style={{ display: 'inline', marginLeft: 10 }}><CopyFilled style={{ fontSize: '16px', color: '#08c' }}/><b style={{ fontSize: 15, marginLeft: 5 }}>{item.recommend}</b></div>
        ]}
      >
        <Meta
          title={item.username}
